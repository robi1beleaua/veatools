@echo off
:: Verifică dacă scriptul este rulat ca administrator
openfiles >nul 2>&1
if %errorlevel% NEQ 0 (
    echo Acest script necesita privilegii de administrator. Reincepe cu drepturi de administrator...
    powershell -Command "Start-Process cmd -ArgumentList '/c %~s0' -Verb RunAs"
    exit
)

:: Incepe procesul de curățare profundă Fortnite
echo Se incepe curățarea și optimizarea Fortnite...
echo.

:: Pasul 1: Închide Fortnite și orice proces legat de acesta
...
:: Pasul 2: Șterge fișierele temporare, loguri și fișiere de crash
del /f /s /q "%LOCALAPPDATA%\FortniteGame\Saved\Logs\*.*"
del /f /s /q "%LOCALAPPDATA%\FortniteGame\Saved\Crashes\*.*"
del /f /s /q "%LOCALAPPDATA%\FortniteGame\Saved\Config\WindowsClient\*.*"
del /f /s /q "%LOCALAPPDATA%\EpicGames\Fortnite\*.*"

:: Pasul 3: Șterge fișierele de actualizare incomplete și cache-ul Epic Games
del /f /s /q "%PROGRAMDATA%\Epic\Fortnite\*.*"
del /f /s /q "%LOCALAPPDATA%\EpicGames\Launcher\Cache\*.*"
del /f /s /q "%LOCALAPPDATA%\EpicGames\Launcher\Saved\*.*"
del /f /s /q "%APPDATA%\EpicGames\Launcher\Saved\*.*"

:: Pasul 4: Șterge fișierele de configurare din registru (doar cele legate de Fortnite)
reg delete "HKCU\Software\Epic Games\Fortnite" /f
reg delete "HKCU\Software\Epic Games\Unreal Engine 4" /f
reg delete "HKCU\Software\Epic Games\Launcher" /f
reg delete "HKCU\Software\Microsoft\Windows\CurrentVersion\Uninstall\Fortnite" /f
reg delete "HKCU\Software\Microsoft\Windows\CurrentVersion\Explorer\Shell Folders" /f

:: Pasul 5: Optimizați fișierele de configurare Fortnite pentru performanță
echo bUseVSync=False > "%LOCALAPPDATA%\FortniteGame\Saved\Config\WindowsClient\GameUserSettings.ini"
echo ResolutionSizeX=1920 >> "%LOCALAPPDATA%\FortniteGame\Saved\Config\WindowsClient\GameUserSettings.ini"
echo ResolutionSizeY=1080 >> "%LOCALAPPDATA%\FortniteGame\Saved\Config\WindowsClient\GameUserSettings.ini"
echo bShowFPS=False >> "%LOCALAPPDATA%\FortniteGame\Saved\Config\WindowsClient\GameUserSettings.ini"

:: Pasul 6: Curățare fișiere de sistem inutile (fără a afecta fișierele esențiale)
...
:: Pasul 7: Optimizare completă Windows (fără a afecta procesele esențiale)
:: Dezactivează efectele vizuale și aplicațiile inutile
echo Se optimizeaza Windows...
powercfg -change -standby-timeout-ac 15
powercfg -change -monitor-timeout-ac 10
powercfg -change -disk-timeout-ac 5
powercfg -change -processor-throttle 100

:: Pasul 8: Curățare a fișierelor de sistem vechi și inutile
del /f /s /q "%SYSTEMDRIVE%\Windows\SoftwareDistribution\Download\*.*"
del /f /s /q "%SYSTEMDRIVE%\Windows\SoftwareDistribution\DataStore\*.*"

:: Pasul 9: Elimină fișierele de log și temporare ale Epic Games Launcher
del /f /s /q "%APPDATA%\EpicGames\Launcher\Saved\Temp\*.*"
del /f /s /q "%LOCALAPPDATA%\EpicGames\Launcher\Saved\Temp\*.*"

:: Pasul 10: Verificare și optimizare fișiere de joc (nu deschide aplicații externe)
echo Verificare integritate fișiere Fortnite...
:: Aici poți adăuga o verificare a fișierelor de joc folosind comenzi interne (Epic Games nu permite verificarea fișierelor din batch direct)

:: Pasul 11: Curățare cache-ul Steam (dacă există)
del /f /s /q "%APPDATA%\Steam\*.*"
del /f /s /q "%LOCALAPPDATA%\Steam\*.*"

:: Pasul 12: Dezactivează servicii inutile care consumă resurse
sc config "DiagTrack" start= disabled
sc stop "DiagTrack"
sc config "wuauserv" start= disabled
sc stop "wuauserv"

:: Pasul 13: Optimizarea proceselor și setărilor de performanță Windows
echo Setări de performanță optimizează acum...
powercfg -change -standby-timeout-ac 15
powercfg -change -monitor-timeout-ac 10
powercfg -change -disk-timeout-ac 5
powercfg -change -processor-throttle 100

:: Pasul 14: Reîmprospătarea completă a sistemului
echo Reîmprospătare completă a sistemului...
start /wait tasklist /fi "imagename eq explorer.exe" > nul
taskkill /f /im explorer.exe
start explorer.exe

:: Pasul 15: Șterge fișierele de la instalările de joc
del /f /s /q "%PROGRAMFILES%\Epic Games\Fortnite\FortniteGame\Saved\*.*"

:: Pasul 16: Curăță și optimizează registrele
...
:: Pasul 17: Șterge fișierele de sistem vechi
del /f /s /q "%WINDIR%\SoftwareDistribution\Download\*.*"
del /f /s /q "%WINDIR%\SoftwareDistribution\DataStore\*.*"

:: Pasul 18: Resetare completă setări Fortnite
echo Se resetează setările Fortnite...
del /f /s /q "%LOCALAPPDATA%\FortniteGame\Saved\Config\WindowsClient\*.*"

:: Pasul 19: Șterge fișiere de la alte platforme de jocuri
del /f /s /q "%APPDATA%\Steam\*.*"
del /f /s /q "%LOCALAPPDATA%\Steam\*.*"

:: Pasul 20: Dezactivează servicii de fundal care afectează performanța
sc config "DiagTrack" start= disabled
sc stop "DiagTrack"
sc config "wuauserv" start= disabled
sc stop "wuauserv"

:: Pasul 21: Întreținere completă a sistemului
echo Întreținerea sistemului este completă!

:: Pasul 13: Închide orice aplicații de fundal care afectează performanța
taskkill /f /im "background_process.exe" >nul 2>&1

:: Pasul 14: Finalizare și optimizare completă
echo Curățarea completă a fost realizată cu succes.
pause
